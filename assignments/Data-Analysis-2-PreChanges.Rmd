...
q1.qqplot <- ggplot(data = mydata, aes(sample = LOG_RATIO)) +
  geom_qq(color='#2c7fb8') +
  geom_qq_line() +
  theme(legend.position = "none") +
...
q1.hist <- ggplot(data = mydata, aes(x = LOG_RATIO)) +
  geom_histogram(binwidth=.02, fill='#2c7fb8') +
  theme(legend.position = "none") +
...
# output skewness and kurtosis
print(paste("Skewness of ratio of shuck weight to volume", skewness(mydata$LOG_RATIO)))
print(paste("Kurtosis of ratio of shuck weight to volume", kurtosis(mydata$LOG_RATIO)))
```
...
bartlett.test(mydata$LOG_RATIO ~ mydata$CLASS)
...
ggplot(data = q6.df, aes(x = volume.value)) +
  geom_line(aes(y = prop.adults), color = "#016450") +
  geom_line(aes(y = prop.infants), color = "#3690c0") +
