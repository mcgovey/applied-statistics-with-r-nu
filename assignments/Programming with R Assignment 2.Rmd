...
ggplot(q2.faithfulDf, aes(x = eruptions, y = waiting, color = criteriaMatch)) +
  geom_point() +
  geom_hline(yintercept = 70, color = "#1b9e77") +
  geom_vline(xintercept = 3.0, color = "#1b9e77") +
  scale_color_brewer(palette="Dark2") +
  labs(x = "Eruptions", y="Wait Time (in minutes)", color = "Criteria", 
       title = "Eruptions vs. Wait Time (Eruption < 3 and Wait Time > 70 highlighted)")

```

(ii) (1 point) What does the plot suggest about the relationship between eruption time and waiting time?

***Answer: The plot above shows a positive correlation that would likely score have a value close to 1 for the Pearson Correlation Coefficient. The plot also shows the data clustered in two areas. The first being relatively short wait time and a small number of eruptions. The second cluster shows that when the wait time is longer, there tend to be a high number of eruptions.    ***
...
# print output
print(paste("The above test shows a p value below 0.05 of", round(q2.kendallCorrelation$p.value, 7), "therefore we reject the null hypothesis and accept the alternative"))
...
boxplot(q3.result1$weight, col = rgb(44, 127, 184, maxColorValue = 255), 
        ylim = c(100, 400), main = "Chicks on experimental diet 1", 
...
boxplot(q3.result3$weight, col = rgb(127, 205, 187, maxColorValue = 255),
        ylim = c(100, 400), main = "Chicks on experimental diet 3", 
        sub = "21 days after birth", ylab = "Weight")
...
ggplot(data = q3.pairedData, aes(y = post, x = pre, color=rgb(127, 205, 187, maxColorValue = 255))) +
  geom_point() +
  geom_line(aes(x = pre, y = pre)) +
  theme(legend.position = "none") +
...
# use below to test results found above
...
hist(sample1$mean, freq = FALSE, col = "darkblue", xlab = "Mean of Sample 1",
     ylim = c(0,0.06), xlim = c(25, 75), main = "Histogram of Sample 1 (n=16) Means")
curve(dnorm(x, mean = sample1stats$Mean, sd = sample1stats$Standard_Deviation), col = "orange", lwd = 2, add = TRUE)

# create histogram for second sample, squished because standard deviation is much smaller
hist(sample2$mean, freq = FALSE, col = "darkblue", xlab = "Mean of Sample 2",
     ylim = c(0,0.2), xlim = c(25, 75), main = "Histogram of Sample 2 (n=64) Means")
curve(dnorm(x, mean = sample2stats$Mean, sd = sample2stats$Standard_Deviation), col = "orange", lwd = 2, add = TRUE)
...
q5.medianBreaks <- median(warpbreaks$breaks)

# create a scatterplot of volume, split by rings
ggplot(data = warpbreaks, aes(x = breaks)) +
  geom_histogram(bins = 30, fill = "#08589e") +
  geom_vline(xintercept = q5.medianBreaks, color = "#7bccc4") +
  theme(legend.position = "none") +
...
# variable for number
number <- ifelse((warpbreaks$breaks < q5.medianBreaks), "Below", "Above")
...
chisqfun(q5.contingencyTable)

print("Compare the output above of the user defined function to the output generated by the chisq.test function below.")
